...
- `SUM(x) OVER ()` with an empty window computes one total across the entire result set and repeats it on every row. The result is that every row gets a grand total to divide against, setting the stage for a percent of total without he needs of additional joints or subqueries.
